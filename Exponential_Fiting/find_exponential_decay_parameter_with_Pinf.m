...
function [P_inf_sel, ...
    ...
    fun_error           = @(Input) search_for_pinf(Input, X_decay, Y_decay, Grad_Y_decay);
    
    try
        Input              	= fmincon(fun_error, X0, A, b, Aeq, beq, lb, ub, nonlcon, options);
        P_inf_sel           = Input;
        [ Est_Y, ...
            Tau_sel]        = compute_exponential_from_Pinf(X_decay, Grad_Y_decay, P_inf_sel, X_decay(1), Y_decay(1));  
        X_decay_Ex          = X_decay;        
        Est_Y_Ex            = Est_Y;
        BP_sel              = Y_decay;
    catch
