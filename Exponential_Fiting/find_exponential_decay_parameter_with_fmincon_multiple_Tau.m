...
function [P_inf_sel, ...
    ...
    fun_error           = @(Input) non_linear_estimate_exponential_parameter_with_multiple_Tau(Input, X_decay, Y_decay);
    
    try
        P_inf_sel     	= fmincon(fun_error, X0, A, b, Aeq, beq, lb, ub, nonlcon, options);
     	[Est_Y, ...
            ~, ...
            X, ...
            Tau_sel, ...
            ~, BP_sel] ...
                        = fit_mutiple_tau(P_inf_sel, X_decay, Y_decay, 50);
        Est_Y_Ex      	= Est_Y;
        X_decay_Ex   	= X; 
    catch
        [P_inf_sel, Tau_sel, ...
